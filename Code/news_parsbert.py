...
def train(epoch, model, dataloader, optimizer, scheduler, device):
    ...
    for data in dataloader:
      ...
      if len(loss_collection) and len(loss_collection) % 100 == 0:
        ...
        print(f'EPOCH [{epoch + 1}/{epochs}] | STEP [{step}/{len(dataloader)}] | Loss {round(loss_, 6)}')
...
if transformer_name == 'roberta':
  transformer_path = 'HooshvareLab/roberta-fa-zwnj-base'
elif transformer_name == 'bert':
  transformer_path = 'HooshvareLab/bert-fa-zwnj-base'
...
dir_ = 'drive/MyDrive/parsbert_0/'
model.load_state_dict(torch.load(os.path.join(dir_, 'best_model.pt'))['model_state_dict'])

output = eval(0, model, eval_dataloader, device)
for key, value in output.items():
  print(key, value)

output = eval(0, model, test_dataloader, device)
for key, value in output.items():
  print(key, value)
